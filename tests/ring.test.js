...
function axioms(name, ring, samples) {
    describe(`${name} — ring axioms`, () => {
        it("a + 0 = a", () => {
            for (const a of samples)
                expect(ring.eq(ring.add(a, ring.zero), a)).toBe(true);
        });
        it("a + (-a) = 0", () => {
            for (const a of samples)
                expect(ring.isZero(ring.add(a, ring.neg(a)))).toBe(true);
        });
        it("a * 1 = a", () => {
            for (const a of samples)
                expect(ring.eq(ring.mul(a, ring.one), a)).toBe(true);
        });
        it("a * 0 = 0", () => {
            for (const a of samples)
                expect(ring.isZero(ring.mul(a, ring.zero))).toBe(true);
        });
        it("a + b = b + a", () => {
            for (const a of samples)
