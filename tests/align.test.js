...
import { coverComplete, pairComponentCount, tripleComponentCount, } from "../src/math/intersection";
import { SCENE_BAD_COVER, SCENE_TORUS_H1 } from "../src/tutorial/scenes";
function withIds(discs) {
...
function badPairs(discs, space) {
    const out = [];
    for (let i = 0; i < discs.length; i++) {
        for (let j = i + 1; j < discs.length; j++) {
            const components = pairComponentCount(space, discs[i], discs[j]);
            if (components > 1)
                out.push([i, j, components]);
        }
    }
    return out;
}
function badTriples(discs, space) {
    const out = [];
    for (let i = 0; i < discs.length; i++) {
        for (let j = i + 1; j < discs.length; j++) {
            for (let k = j + 1; k < discs.length; k++) {
                ...
                if (components > 1)
                    out.push([i, j, k, components]);
            }
        }
    }
    return out;
}
function coveredGridPixels(discs) {
    const min = -6;
    const max = 6;
    const n = 80;
...
function uniqueGridPixelCounts(discs) {
    const min = -6;
    const max = 6;
    const n = 80;
    const step = (max - min) / n;
