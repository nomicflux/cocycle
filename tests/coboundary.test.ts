...
import type { Nerve } from "../src/state/types";
...
function isZeroMatrix(M: number[][]): boolean {
...
function partialSimplexNerve(n: number, maxDim: number): Nerve {
  const byDim: number[][][] = [];
  for (let d = 0; d <= maxDim; d++) {
    const size = d + 1;
    const sims: number[][] = [];
    const choose = (start: number, cur: number[]): void => {
      if (cur.length === size) { sims.push([...cur]); return; }
