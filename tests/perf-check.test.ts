...
type Scene = { id: string; space: Space; discs: Array<{ cx: number; cy: number; r: number; region?: string }> };

const tutorialScenes: Scene[] = [
  { id: "scene_projective_rp2", space: TutorialScenes.SCENE_PROJECTIVE_RP2.space ?? "planar" as Space, discs: TutorialScenes.SCENE_PROJECTIVE_RP2.discs },
  { id: "scene_wedge2", space: TutorialScenes.SCENE_WEDGE2.space ?? "planar" as Space, discs: TutorialScenes.SCENE_WEDGE2.discs },
  { id: "scene_torus_h1", space: TutorialScenes.SCENE_TORUS_H1.space ?? "planar" as Space, discs: TutorialScenes.SCENE_TORUS_H1.discs },
];
...
it("perf: cohomology per ring across all presets", () => {
  for (const preset of [...allPresets, ...tutorialScenes]) {
    const discs: Disc[] = preset.discs.map((d, i) => ({
      ...
      region: (d as { region?: string }).region as Disc["region"],
    }));
    const nerve = buildNerve(discs, 3, { space: ('space' in preset && preset.space ? preset.space : "planar") as Space });
    const Ns = [
