...
function matMul(A: RingElement[][], B: RingElement[][], ring: Ring): RingElement[][] {
  if (A.length === 0) return [];
  const k = A[0].length;
  if (k === 0 || B.length === 0) {
    return A.map(() => new Array<RingElement>(B[0]?.length ?? 0).fill(ring.zero));
  }
  const m = A.length;
  const n = B[0].length;
  const out: RingElement[][] = Array.from({ length: m }, () =>
    new Array<RingElement>(n).fill(ring.zero),
  );
  for (let i = 0; i < m; i++) {
    for (let j = 0; j < n; j++) {
      let acc = ring.zero;
      for (let l = 0; l < k; l++) acc = ring.add(acc, ring.mul(A[i][l], B[l][j]));
      out[i][j] = acc;
    }
  }
  return out;
...
function matEq(A: RingElement[][], B: RingElement[][], ring: Ring): boolean {
  if (A.length !== B.length) return false;
  for (let i = 0; i < A.length; i++) {
    if ((A[i].length) !== (B[i].length)) return false;
    for (let j = 0; j < A[i].length; j++) {
      if (!ring.eq(A[i][j], B[i][j])) return false;
    }
  }
  return true;
}

function verifyFactorization(M: RingElement[][], ring: Ring): void {
