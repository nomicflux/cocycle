...
export const CHAPTERS: Chapter[] = [
  ...
  },
  {
    ...
    title: "Cocycles",
    prose:
      "You've now seen δ produce nonzero output. The *opposite* situation is more interesting: cochains where δ produces *nothing*.\n\nA k-cochain c is a **cocycle** if δc = 0. Equivalently, c lies in the *kernel* of δ. The group of k-cocycles is written **Zᵏ** (a subgroup of Cᵏ).\n\nFor 0-cochains, cocycle means δc = 0 on every edge, which forces c to be constant on each connected component of the nerve. Boring.\n\nFor 1-cochains, it gets rich. δc = 0 on an edge cochain means: 'around every triangle, the alternating sum of the three edge values cancels'. Geometrically, triangles are the discrete analogue of small loops, so a 1-cocycle is one whose 'integral around every loop' vanishes — exactly the discrete cousin of a *closed* 1-form in calculus.\n\nWhy fixate on cocycles? Because cohomology is going to be built from them. H¹ will end up being 'all 1-cocycles, modulo a notion of equivalence' — and that equivalence is *exactly* 'differs by something of the form δb'. So cocycles are the raw material; the next chapter quotients them.\n\nSwitch to the H¹ tab and try to put nonzero integers on edges such that the 'δc ≠ 0' box completely vanishes.",
    scene: scenes.SCENE_TORUS_RING,
    unlocks: ["h1"],
    goal: P.hasH1Cocycle,
    goalHint:
      "Click the H¹ tab in the Cohomology panel — the editor now has one row per edge. A short way to land a 1-cocycle on this torus cover: pick a *closed loop* of edges (one that wraps around the torus) and give each edge value 1, leaving all other edges at 0. If you've chosen a true loop, no triangle's alternating sum will fire, and the yellow δc box disappears.",
  },
  {
    id: "non-cocycles",
    title: "Coboundaries and non-cocycles",
    prose:
      "Some cocycles are 'trivial' — they're cocycles for a *stupid* reason: they're equal to δb for some lower-degree cochain b. Such cochains are automatically cocycles because δ∘δ = 0.\n\nThese trivially-cocycle cochains are called **coboundaries** (noun, plural: coboundaries; image of δ). The group of k-coboundaries is written **Bᵏ**. By construction Bᵏ ⊆ Zᵏ.\n\n(Yes — 'coboundary' is doing double duty. The *operator* δ is also called *the coboundary*. The *noun* coboundary refers to a specific cochain in the image of δ. Context disambiguates, and the dual usage is universal.)\n\nWhy single out coboundaries? Because they're the cocycles that *fail to carry global information*. If c = δb, then c was 'generated' by a simpler labelling b; it's the gradient of a function, in the discrete sense. The interesting cocycles are the ones that *aren't* coboundaries — those will be the actual content of H¹.\n\nThe opposite of a cocycle is a **non-cocycle**: a cochain with δc ≠ 0. Take the cocycle you built last chapter and break it by hand.",
    scene: scenes.SCENE_TORUS_RING,
    unlocks: ["h2"],
