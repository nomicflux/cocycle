...
export const CHAPTERS: Chapter[] = [
  ...
  },
  {
    ...
    prose:
      "Cohomology has multiplication as well. The product of a p-cochain α and a q-cochain β is a (p+q)-cochain α ⌣ β, the **cup product**.\n\nThe recipe is simplex-by-simplex. To evaluate (α ⌣ β) on a (p+q)-simplex σ = {v₀, …, v_{p+q}}, split σ at the vertex in position p:\n\n• **front face** = the first p+1 vertices, {v₀, …, v_{p}} — feed it to α.\n• **back face** = the last q+1 vertices, {v_{p}, …, v_{p+q}} — feed it to β.\n• **pivot** = the shared vertex v_{p}, where front meets back.\n\nThen (α ⌣ β)(σ) = α(front) · β(back), the product taken in the coefficient ring (the integers for now).\n\nTake one triangle σ = {0, 1, 2} and two 1-cochains α {0,1} & β {1,2}. Put a number on both edges: suppose α({0,1}) = 1 and β({1,2}) = 1. A 1 ⌣ 1 product splits at vertex p = 1:\n\n• front face = {0, 1}  →  α({0,1}) = 1\n• pivot = vertex 1\n• back face = {1, 2}  →  β({1,2}) = 1\n\nSo (α ⌣ β)({0,1,2}) = α({0,1}) · β({1,2}) = 1 · 1 = 1.\n\nNow reproduce that on this cover. Tick **Cup product** in the topbar to open the cup panel. Each factor is captured from the cochain you are currently **focused** on, so first move the **Focus** selector (the ‹ › row near the top of the Cohomology panel) until it reads **Focus: C¹ → C²**. That focuses the 1-cochain that lives on edges — exactly what A and B must be. (Leave Focus on C⁰ → C¹ and the buttons capture the empty C⁰ instead, so A ⌣ B comes out 0.)\n\n**A is α:** in the C¹ editor type **1** in the **{0,1}** row (leave {0,2} and {1,2} at 0), then click **Set current as A** — the cup panel should now read 'A (C¹)'.\n**B is β:** set the {0,1} row back to 0, type **1** in the **{1,2}** row, then click **Set current as B**.\n\nThe panel prints A ⌣ B on {0,1,2}, and its single value is A({0,1}) · B({1,2}) = 1 · 1 = 1 — the calculation above, run by the app. Hover the triangle in the 'Nerve (geometric)' panel: its front edge {0,1}, pivot vertex 1, and back edge {1,2} light up — the two edges that carried A and B.\n\nThe open cover view now shows two marks. An arrow sits in the intersection of discs 1 and 2, pointing from the low-index disc to the high-index one (1 → 2) with length set by its value (here, 1). A purple dot labelled 1 sits in the small patch where all three discs overlap at once — that is the cup result A ⌣ B on the triangle {0,1,2}, a 2-cochain drawn at the triple overlap, and its value is exactly the 1 · 1 = 1 you computed by hand.",
    scene: scenes.SCENE_CUP_TRIANGLE,
    unlocks: ["space-selector", "presets", "snapshots"],
    goal: P.usedCupProduct,
    goalHint:
      "Tick 'Cup product' in the topbar. Move the Focus selector (the ‹ › row near the top of the Cohomology panel) until it reads 'Focus: C¹ → C²' — otherwise 'Set current as A/B' captures the empty C⁰ and the cup is 0. In the C¹ editor type 1 in the {0,1} edge row (others 0) and click 'Set current as A' (it should read 'A (C¹)'). Then set {0,1} back to 0, type 1 in the {1,2} edge row, and click 'Set current as B'. The cup panel prints A ⌣ B = 1 on {0,1,2}; the goal fires once a nonzero 1 ⌣ 1 cup has been formed.",
  },
  {
