...
export function cumulativeUnlocks(step) {
    const set = new Set();
    const last = Math.min(step, CHAPTERS.length - 1);
    for (let i = 0; i <= last; i++) {
        for (const f of CHAPTERS[i].unlocks)
            set.add(f);
    }
    return set;
}
