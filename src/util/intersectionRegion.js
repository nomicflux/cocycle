...
function inAllOthers(simDiscs, skip, p) {
    for (let j = 0; j < simDiscs.length; j++) {
        if (j === skip)
            continue;
        if (!pointInDisc(p[0], p[1], simDiscs[j]))
            return false;
    }
    return true;
}
...
export function intersectionPolygon(simDiscs) {
    if (simDiscs.length === 0)
        return [];
    if (simDiscs.length === 1)
        return sampleCircle(simDiscs[0], 60);
    const collected = [];
    simDiscs.forEach((d, i) => {
        for (const p of sampleCircle(d, 80)) {
            if (inAllOthers(simDiscs, i, p))
                collected.push(p);
        }
    });
...
export function intersectionCentroid(simDiscs) {
...
export function intersectionComponents(space, simDiscs) {
    if (simDiscs.length === 0)
        return [];
    if (simDiscs.length === 1)
        return [intersectionPolygon(simDiscs)];
    if (space === "wedge2") {
    ...
    for (let i = 1; i < simDiscs.length; i++) {
        if (simDiscs[i].r < simDiscs[anchorIdx].r)
            anchorIdx = i;
    }
    const elements = deckElements(space);
    const others = [];
    for (let i = 0; i < simDiscs.length; i++)
        if (i !== anchorIdx)
            others.push(simDiscs[i]);
    const pieces = [];
    const stack = [simDiscs[anchorIdx]];
    const recur = (depth) => {
