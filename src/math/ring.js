...
import { solveLinearEuclidean, cokernelStructureEuclidean, kernelBasisEuclidean, } from "./linalg-euclidean";
import { solveLinearOverField, cokernelStructureOverField, kernelBasisOverField, } from "./rank";
import { solveLinearZn, cokernelStructureZn, kernelBasisZn, } from "./linalg-zn";
// ------------------------ Z ------------------------
const Z_ZERO = [0];
const Z_ONE = [1];
export const ZRing = {
    ...
    quotRem: (a, b) => {
        if (b[0] === 0)
            throw new Error("Z: quotRem by zero");
...
export function ZpRing(p) {
    ...
    const r = {
        ...
        quotRem: isField
            ? (a, b) => {
                if (b[0] === 0)
                    throw new Error(`Z/${p}: quotRem by zero`);
        ...
        applyUCT: isField
            ...
            : (intHk, tNext) => {
                ...
                for (const d of intHk.torsion)
                    fold(d);
                for (const e of tNext)
                    fold(e);
                return { rank, torsion };
            },
    };
...
export const QRing = {
    ...
    parse: (s) => {
        ...
        if (t.includes("/")) {
            ...
            if (!Number.isFinite(ni) || !Number.isFinite(di) || di === 0)
                return null;
    ...
    quotRem: (a, b) => {
        if (b[0] === 0)
            throw new Error("Q: quotRem by zero");
...
export function qInv(x) {
    if (x[0] === 0)
        throw new Error("Q: division by zero");
...
function ziQuotRem(a, b) {
    const [a1, a2] = a;
    const [b1, b2] = b;
    const nb = b1 * b1 + b2 * b2;
    if (nb === 0)
        throw new Error("Z[i]: quotRem by zero");
...
function parseComplex(s, imagSym) {
    ...
    if (splitIdx.length === 0)
        pieces = [t];
    else if (splitIdx.length === 1)
        pieces = [t.slice(0, splitIdx[0]), t.slice(splitIdx[0])];
    else
        return null;
    let a = 0, b = 0;
...
function zwQuotRem(a, b) {
    const [a1, a2] = a;
    const [b1, b2] = b;
    const nb = b1 * b1 - b1 * b2 + b2 * b2;
    if (nb === 0)
        throw new Error("Z[ω]: quotRem by zero");
...
export function zToInt(x) {
    return x[0];
}
// Display a ring element with a leading "+" if positive (Z, Q).
export function signedFormat(ring, x) {
    if (ring.isPositive(x))
        return `+${ring.format(x)}`;
    return ring.format(x);
}
// Inverse over a field ring. Throws on non-field or on dividing by zero.
export function fieldInv(ring, x) {
    if (ring.spec.kind === "Q")
        return qInv(x);
    if (ring.spec.kind === "Zp" && isPrime(ring.spec.p))
        return zpInv(ring.spec, x);
