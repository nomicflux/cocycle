...
export function solveLinearZn(A, b, n) {
    const m = A.length;
    const nCols = m === 0 ? 0 : A[0].length;
    if (m === 0)
        return b.length === 0 ? [] : null;
    if (nCols === 0)
        return b.every((x) => x[0] === 0) ? [] : null;
...
export function cokernelStructureZn(A, n) {
    const m = A.length;
    const nCols = m === 0 ? 0 : A[0].length;
    if (m === 0 || nCols === 0)
...
export function kernelBasisZn(A, nCols, n) {
    if (nCols === 0)
        return [];
    ...
    for (let i = 0; i < nCols; i++) {
        ...
        if (annihilator === n)
            continue; // d is a unit mod n → no kernel contribution
        const col = snf.V.map((row) => mod(row[i] * annihilator, n));
        basis.push(col.map((x) => [x]));
    }
    return basis;
}
