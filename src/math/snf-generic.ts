...
function eye(n: number, ring: Ring): RingElement[][] {
  return Array.from({ length: n }, (_, i) =>
    Array.from({ length: n }, (_, j) => (i === j ? ring.one : ring.zero)),
  );
}

function clone(M: RingElement[][]): RingElement[][] {
  return M.map((row) => row.slice());
}

function swapRows(M: RingElement[][], i: number, j: number): void {
  if (i !== j) [M[i], M[j]] = [M[j], M[i]];
}

function swapCols(M: RingElement[][], i: number, j: number): void {
  if (i === j) return;
  for (const row of M) [row[i], row[j]] = [row[j], row[i]];
}

function addRow(
  M: RingElement[][], src: number, dst: number, mult: RingElement, ring: Ring,
): void {
...
function findSmallestNonzero(
  D: RingElement[][], i0: number, m: number, n: number, ring: Ring,
): [number, number] | null {
  let r = -1, c = -1, best = Infinity;
  for (let r2 = i0; r2 < m; r2++) {
    for (let c2 = i0; c2 < n; c2++) {
      if (ring.isZero(D[r2][c2])) continue;
      const v = ring.norm(D[r2][c2]);
      if (v < best) { best = v; r = r2; c = c2; }
    }
  }
  return r === -1 ? null : [r, c];
}
...
function reduceColumn(
  D: RingElement[][], U: RingElement[][], i: number, m: number, ring: Ring,
): boolean {
  let progress = false;
  for (let r = i + 1; r < m; r++) {
    if (ring.isZero(D[r][i])) continue;
...
function reduceRow(
  D: RingElement[][], V: RingElement[][], i: number, n: number, ring: Ring,
): boolean {
  let progress = false;
  for (let c = i + 1; c < n; c++) {
    if (ring.isZero(D[i][c])) continue;
...
function fixDivisibility(
  D: RingElement[][], U: RingElement[][],
  i: number, m: number, n: number, ring: Ring,
): boolean {
  for (let r = i + 1; r < m; r++) {
    for (let c = i + 1; c < n; c++) {
...
function pivotAt(
  D: RingElement[][], U: RingElement[][], V: RingElement[][],
  i: number, m: number, n: number, ring: Ring,
): void {
  let changed = true;
  while (changed) {
    changed = reduceColumn(D, U, i, m, ring);
    if (reduceRow(D, V, i, n, ring)) changed = true;
  }
}

export function smithNormalFormGeneric(M_in: RingElement[][], ring: Ring): SNFGen {
  const m = M_in.length;
  ...
  while (i < Math.min(m, n)) {
    ...
    if (fixDivisibility(D, U, i, m, n, ring)) i++;
  }
  let rank = 0;
  for (let k = 0; k < Math.min(m, n); k++) if (!ring.isZero(D[k][k])) rank++;
  return { U, V, D, rank };
}
