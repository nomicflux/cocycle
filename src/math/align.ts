...
import { deckApplyDisc, deckElements, DECK_ID, type DeckElem } from "./deck";
...
function torusDist2(x1: number, y1: number, x2: number, y2: number): number {
...
function escapeDelta(c_i: Vec, gSrc: Vec, sum: number, g: DeckElem): Vec {
  const v = diff(c_i, gSrc);
  const d = norm(v);
  if (d < 1e-9) return { x: g.sx * (sum + TANGENT_MARGIN), y: 0 };
  const eps = sum - d + TANGENT_MARGIN;
  return { x: -g.sx * eps * v.x / d, y: -g.sy * eps * v.y / d };
}

function applyMove(d: Disc, dx: number, dy: number, space: Space): Disc {
...
type MoveChoice = { idx: number; disc: Disc; count: number; dist2: number };

function considerMove(
  best: MoveChoice | null,
  discs: Disc[],
  idx: number,
  cand: Disc,
  count: number,
...
function pairEscapeMoves(
  ...
  const fixedCenter: Vec = { x: fixed.cx, y: fixed.cy };
  const sum = fixed.r + moving.r;
  const out: Disc[] = [];
  for (const g of elements) {
    const lifted = deckApplyDisc(g, moving);
    if (!pairIntersects(fixed, lifted)) continue;
...
function fixPair(
  discs: Disc[], i: number, j: number, elements: DeckElem[], space: Space,
): boolean {
  for (let it = 0; it < FIX_MAX_ITERS; it++) {
    const before = pairComponentCount(space, discs[i], discs[j]);
    if (before <= 1) return false;
    let best: MoveChoice | null = null;
    for (const [fixedIdx, movingIdx] of [[i, j], [j, i]] as const) {
      for (const cand of pairEscapeMoves(discs, fixedIdx, movingIdx, elements, space)) {
        ...
        if (count < before) best = considerMove(best, discs, movingIdx, cand, count);
      }
    }
    if (best === null) return false;
    discs[best.idx] = best.disc;
    if (best.count <= 1) return true;
  }
  return true;
}

function pushEscapeMove(
  out: Array<{ idx: number; disc: Disc }>,
  discs: Disc[],
  idx: number,
  fixed: Vec,
  lifted: Vec,
...
function tripleEscapeMovesForAnchor(
  discs: Disc[],
  anchorIdx: number,
  bIdx: number,
  cIdx: number,
  elements: DeckElem[],
  space: Space,
): Array<{ idx: number; disc: Disc }> {
  const out: Array<{ idx: number; disc: Disc }> = [];
  const anchor = discs[anchorIdx];
  const b = discs[bIdx];
  const c = discs[cIdx];
  const aCenter: Vec = { x: anchor.cx, y: anchor.cy };
  for (const gb of elements) {
    const lb = deckApplyDisc(gb, b);
    const lbCenter: Vec = { x: lb.cx, y: lb.cy };
    for (const gc of elements) {
      const lc = deckApplyDisc(gc, c);
      if (!tripleIntersects(anchor, lb, lc)) continue;
      const lcCenter: Vec = { x: lc.cx, y: lc.cy };
...
function fixTriple(
  discs: Disc[], i: number, j: number, k: number,
  elements: DeckElem[], space: Space,
): boolean {
  for (let it = 0; it < FIX_MAX_ITERS; it++) {
    ...
    for (const [anchorIdx, bIdx, cIdx] of anchorOrders) {
      for (
        const { idx, disc } of tripleEscapeMovesForAnchor(
          discs, anchorIdx, bIdx, cIdx, elements, space,
        )
      ) {
        ...
        if (count < before) best = considerMove(best, discs, idx, disc, count);
      }
    }
    if (best === null) return false;
    discs[best.idx] = best.disc;
...
type Phase1Result = { discs: Disc[]; changed: boolean };

function phase1Pass(discs: Disc[], space: Space, elements: DeckElem[]): boolean {
  let touched = false;
...
function phase1Sweep(discs: Disc[], space: Space): Phase1Result {
...
function pixelIdx(ix: number, iy: number): number {
  const wx = ((ix % GRID_N) + GRID_N) % GRID_N;
  const wy = ((iy % GRID_N) + GRID_N) % GRID_N;
  return wx * GRID_N + wy;
}

function pixelXY(ix: number, iy: number): Vec {
  return { x: -HALF + (ix + 0.5) * STEP, y: -HALF + (iy + 0.5) * STEP };
}

function nearestGridIdx(c: number): number {
  const ix = Math.round((c + HALF) / STEP - 0.5);
  return ((ix % GRID_N) + GRID_N) % GRID_N;
}

const footprintCache = new Map<number, Array<[number, number]>>();
type FootprintSpan = { dy: number; minDx: number; maxDx: number };
type FootprintSpans = { spans: FootprintSpan[]; area: number; wraps: boolean };
const footprintSpanCache = new Map<number, FootprintSpans>();

function discFootprint(r: number): Array<[number, number]> {
  ...
  for (let dx = -max; dx <= max; dx++) {
    for (let dy = -max; dy <= max; dy++) {
      if ((dx * STEP) ** 2 + (dy * STEP) ** 2 <= r2) out.push([dx, dy]);
    }
  }
...
function discFootprintSpans(r: number): FootprintSpans {
  const key = Math.round(r * 1e4);
  const hit = footprintSpanCache.get(key);
  if (hit) return hit;
  const max = Math.ceil(r / STEP) + 1;
  const r2 = r * r;
  const spans: FootprintSpan[] = [];
  let area = 0;
...
function coverBitmap(d: Disc, elements: DeckElem[], space: Space): Uint8Array {
...
type Coverage = { perDisc: Uint8Array[]; total: Uint16Array };

function computeCoverage(discs: Disc[], space: Space): Coverage {
  const elements = deckElements(space);
  const perDisc: Uint8Array[] = [];
  const total = new Uint16Array(PIXELS);
  for (const d of discs) {
    const bm = coverBitmap(d, elements, space);
    perDisc.push(bm);
    for (let p = 0; p < PIXELS; p++) total[p] += bm[p];
  }
...
function scoreCandidateBitmap(
  bm: Uint8Array,
  total: Uint16Array,
  selfBm: Uint8Array,
): { overlap: number; empty: number } {
  let overlap = 0;
  let empty = 0;
  for (let p = 0; p < PIXELS; p++) {
    if (bm[p] === 0) continue;
    if (total[p] - selfBm[p] > 0) overlap += 1;
    else empty += 1;
  }
  return { overlap, empty };
}

function scoreTorusCandidate(
  ix: number,
  iy: number,
...
function buildOtherCoverageRowPrefix(total: Uint16Array, selfBm: Uint8Array): Uint16Array {
...
function rowPrefixRange(prefix: Uint16Array, y: number, minX: number, maxX: number): number {
  ...
  if (start <= end) return prefix[base + end + 1] - prefix[base + start];
  return prefix[base + GRID_N] - prefix[base + start] + prefix[base + end + 1];
}

function scoreTorusCandidateByRows(
  ix: number,
  iy: number,
  footprint: FootprintSpans,
  otherPrefix: Uint16Array,
): { overlap: number; empty: number } {
  let overlap = 0;
  for (const span of footprint.spans) {
    overlap += rowPrefixRange(
      otherPrefix,
      iy + span.dy,
      ix + span.minDx,
      ix + span.maxDx,
...
function stepTowardTarget(
  ...
  const np = normalizePosition(
    disc.cx + T * wrappedStep(disc.cx, target.x),
    disc.cy + T * wrappedStep(disc.cy, target.y),
    space,
  );
  return { ...disc, cx: np.cx, cy: np.cy, region: np.region ?? disc.region };
}

function bestLegalGridMove(
  discs: Disc[],
  idx: number,
  total: Uint16Array,
  selfBm: Uint8Array,
  ...
  for (let ix = 0; ix < GRID_N; ix++) {
    for (let iy = 0; iy < GRID_N; iy++) {
      let score: { overlap: number; empty: number };
  ...
  for (const scored of candidates) {
    ...
    let candidateLocalExcess = movedDiscComponentExcess(
      discs, space, idx, candidate, previousLocalExcess,
    );
    while (candidateLocalExcess > previousLocalExcess && localT > CONVERGENCE_TEMP) {
      ...
      candidateLocalExcess = movedDiscComponentExcess(
        discs, space, idx, candidate, previousLocalExcess,
      );
    }
    const nextExcess = allowedExcess - previousLocalExcess + candidateLocalExcess;
    if (nextExcess > allowedExcess) continue;
    ...
    if (candidateScore.empty === 0) continue;
    return {
      disc: candidate,
      excess: nextExcess,
...
function componentExcess(discs: Disc[], space: Space): number {
  let excess = 0;
  const n = discs.length;
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
...
function movedDiscComponentExcess(
  discs: Disc[],
  space: Space,
  idx: number,
  disc: Disc = discs[idx],
...
function wrappedStep(from: number, to: number): number {
  let delta = to - from;
  if (delta > HALF) delta -= TORUS_PERIOD;
  if (delta < -HALF) delta += TORUS_PERIOD;
  return delta;
}

type Phase2Result = { discs: Disc[]; changed: boolean };

function phase2Lloyd(discs: Disc[], space: Space, T: number): Phase2Result {
  const out = discs.map((d) => ({ ...d }));
  const elements = deckElements(space);
  ...
  for (let i = 0; i < out.length; i++) {
    const best = bestLegalGridMove(
      out, i, cov.total, cov.perDisc[i], elements, space, T, allowedExcess,
    );
...
function isGood(discs: Disc[], space: Space): boolean {
  if (!coverComplete(discs, space)) return false;
  const n = discs.length;
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      if (pairComponentCount(space, discs[i], discs[j]) > 1) return false;
    }
  }
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      for (let k = j + 1; k < n; k++) {
        if (tripleComponentCount(space, discs[i], discs[j], discs[k]) > 1) return false;
      }
    }
  }
  return true;
...
export function alignDiscsToGoodCover(initial: Disc[], space: Space): Disc[] {
  if (initial.length <= 1) return initial;
  if (space === "planar" || space === "wedge2") return initial;
  ...
  for (let step = 0; step < STEPS; step++) {
    ...
    cur = phase1.discs;
    if (isGood(cur, space)) return cur;
    if (!phase2.changed && !phase1.changed) break;
    T *= COOLING;
    if (T < CONVERGENCE_TEMP) break;
  }
