...
function matVecMul(
  M: RingElement[][], v: RingElement[], rows: number, cols: number, ring: Ring,
): RingElement[] {
  const out: RingElement[] = new Array(rows).fill(ring.zero);
  for (let i = 0; i < rows; i++) {
    let acc = ring.zero;
    for (let j = 0; j < cols; j++) acc = ring.add(acc, ring.mul(M[i][j], v[j]));
    out[i] = acc;
  }
  return out;
}
...
export function solveLinearEuclidean(
  ...
  const y: RingElement[] = new Array(n).fill(ring.zero);
  const r = Math.min(m, n);
  for (let i = 0; i < r; i++) {
    const d = snf.D[i][i];
    if (ring.isZero(d)) {
      if (!ring.isZero(Ub[i])) return null;
    } else {
      const { q, r: rem } = ring.quotRem(Ub[i], d);
      if (!ring.isZero(rem)) return null;
      y[i] = q;
    }
  }
  for (let i = r; i < m; i++) if (!ring.isZero(Ub[i])) return null;
...
export function cokernelStructureEuclidean(
  A: RingElement[][], ring: Ring,
): { rank: number; torsion: number[] } {
  const m = A.length;
  const n = m === 0 ? 0 : A[0].length;
  if (m === 0 || n === 0) return { rank: m, torsion: [] };
  const snf = smithNormalFormGeneric(A, ring);
  const torsion: number[] = [];
  let p = 0;
  for (let k = 0; k < Math.min(m, n); k++) {
    const d = snf.D[k][k];
    if (ring.isZero(d)) continue;
    p++;
    const norm = ring.norm(d);
    if (norm > 1) torsion.push(norm);
  }
...
export function kernelBasisEuclidean(
  A: RingElement[][], nCols: number, ring: Ring,
): RingElement[][] {
  if (nCols === 0) return [];
